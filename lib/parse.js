...
const isContrastChecker = require('is-contrast-checker');
...
/**
 * Convert gradients doc block to a collection of gradient objects
 *
 * @private
 * @param {String} text paragraph from a comment block.
 * @returns {Array} collection of gradient objects
 */
const parseGradients = function(text) {
  ...
  while (test !== null) {
    const gradient = {};
    gradient.name = test[1].trim();
    gradient.sass = test[2].trim();
    gradient.css = test[3].trim();
...
const parseColors = function(text) {
  ...
  while (test !== null) {
    const color = {};
    color.sass = test[1].trim();
    color.hex = test[2].trim();
    color.name = test[3].trim();
    test = regex.exec(text);
    colors.push(color);
  }
...
/**
 * Convert contrast doc block to a collection of contrast color objects
 *
 * @private
 * @param {String} text paragraph from a comment block.
 * @returns {Array} collection of color objects `{name: '…', color: '…'}`
 */
const parseContrast = function(text) {
  // Replace runs of white space with a single space.
  text = text.trim();
...
/**
 * Convert icons doc block to a collection of icon objects
 *
 * @private
 * @param {String} text paragraph from a comment block.
 * @returns {Array} collection of icon objects
 */
const parseIcons = function(text) {
  // Replace runs of white space with a single space.
  text = text.trim();
  ...
  while (test !== null) {
    const icon = {};
    icon.name = test[1].trim();
    icon.character = test[2].trim();
    test = regex.exec(text);
    icons.push(icon);
  }
...
/**
 * Convert svg icons doc block to a collection of icon objects
 *
 * @private
 * @param {String} text paragraph from a comment block.
 * @returns {Array} collection of icon objects
 */
const parseSvgIcons = function(text) {
  // Replace runs of white space with a single space.
  text = text.trim();
...
const parse = function(input, options) {
  ...
  for (let file of files) {
    ...
    for (let comment of comments) {
      ...
      processProperty.call(newSection, paragraphs, 'Gradients', parseGradients);
      processProperty.call(newSection, paragraphs, 'Colors', parseColors);
      processProperty.call(newSection, paragraphs, 'Icons', parseIcons);
      processProperty.call(newSection, paragraphs, 'SVGIcons', parseSvgIcons);
      processProperty.call(newSection, paragraphs, 'Contrast', parseContrast);
