...
/**
 * Convert programs doc block to a collection of program objects
 *
 * @private
 * @param {String} text paragraph from a comment block.
 * @returns {Array} collection of icon objects
 */
const parsePrograms = function(text) {
  // Replace runs of white space with a single space.
  text = text.trim();
  ...
  for (var i = 0; i < matches.length; i++) {
    var $this = matches[i];
    var arrayMatch = $this.split(' : ');
    var program = {};
    program.name = arrayMatch[0];
    program.dash = arrayMatch[1];
...
const parse = function(input, options) {
  ...
  for (let file of files) {
    ...
    for (let comment of comments) {
      ...
      processProperty.call(newSection, paragraphs, 'Programs', parsePrograms);
