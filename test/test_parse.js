  ...
  context('given options', function() {
    ...
    describe('.colors', function() {
      ...
      it('should find colors property', function(done) {
        ...
        expect(colors[0].name).to.equal('Dark Gray');
        expect(colors[0].hex).to.equal('#333');
        expect(colors[0].sass).to.equal('primary-color(gray, shade1)');
        expect(colors[1].name).to.equal('Gray');
        expect(colors[1].hex).to.equal('#666');
        expect(colors[1].sass).to.equal('primary-color(gray)');
        expect(colors[2].name).to.equal('Light Gray');
        expect(colors[2].hex).to.equal('#ccc');
        expect(colors[2].sass).to.equal('primary-color(gray, tint1)');
        expect(section.description()).to.equal('');
        done();
      });
    });
